...
<body>
	<h1>JSON (Javascript Object Notation : 객체 표기 방법)</h1>
	<h1>이기종 간의 표준 데이터 통신 방법</h1>
	<ol>
		<li>XML(Powerful)</li>
		<li>JSON(Powerful)</li>
		<li>그 외 : 콤마 separator(ex=> 10,총무부,서울)</li>
	</ol>
	<hr>
	<h1>AJAX (Asynchronous Javascript And XML : 비동기 자바스크립트 데이터 통신 기술)</h1>

	<button id="ajax" class="btn btn-primary" style="margin-right: 30px;">getJSON dept</button>
	<button id="ajax" class="btn btn-primary" style="margin-right: 30px;">getJSON depts</button>
	
	<button id="ajax" class="btn btn-primary" style="margin-right: 30px;">getJSON dept + handlebars</button>
	<button id="ajax" class="btn btn-primary" style="margin-right: 30px;">getJSON depts + handlebars</button>
	...
	<script type="text/javascript">
		...
		$('button').eq(1).on('click', function() {
					...
					$.getJSON("/study/json/depts.json", function(data) {
						...
						var lis="";
						for (var i = 0; i < data.length; i++) {
							lis += '<li class="list-group-item list-group-item-sucess">' + data[i].deptno + ", " + data[i].dname + ", " + data[i].loc + "</li>";
						}
